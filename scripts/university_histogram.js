var universityHistogram = {
	width: 0,
	height: 0,
	initialize: function(){
		var self = this;
		var divWidth = $("#univ-histogram").width();
		var divHeight = $("#univ-histogram").height();
		var margin = {right: 40, left: 50, top: 30, bottom: 30},
			width = divWidth - margin.right - margin.left,
			height = divHeight - margin.top - margin.bottom;
		self.width = width;
		self.height = height;
		var svg = d3.select("#svg-univ-histogram")
		.attr("width", width + margin.left + margin.right)
		.attr("height", height + margin.top + margin.bottom)
		.append("g")
		.attr("id", "g-univ-histogram")
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")");
		self.render();
	},
	render: function(){
		var self = this;
		var svg = d3.select("#g-univ-histogram");
